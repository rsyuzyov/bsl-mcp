        ...
        while not self.stop_event.is_set():
            if not self.status.running:
                try:
                    ...
                    if has_changes:
                        print(f"[{self.config.name}] Обнаружены изменения, запуск индексации...")
                        self.engine.incremental_reindex(self.status)
                except Exception as e:
                    print(f"[{self.config.name}] Ошибка в цикле обслуживания: {e}")
...
class IBManager:
    ...
    def _init_ib(self, ib_conf: IBConfig):
        ...
        try:
            print(f"Инициализация ИБ: {ib_conf.name}...")
            engine = IndexEngine(
            ...
            # Searcher будет инициализирован с engine.client
            searcher = HybridSearch(engine.client, collection_name=f"code_{ib_conf.name}", model_name=ib_conf.embedding_model)
            ...
            print(f"ИБ {ib_conf.name} инициализирована")
        except Exception as e:
            print(f"Ошибка инициализации ИБ {ib_conf.name}: {e}")
